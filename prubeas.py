...
mstage1 = pg.mixer.music.load("music\musicstage1.wav")
pg.mixer.music.play()
...
for i in range(10): #número de asteroides
    asteroid = Asteroid()
...
while running:
    ...
    for hit in hits:
        ...
        """asteroid = Asteroid()
        asteroid_list.add(asteroid) #añadimos los asteorides a la lista
        all_sprite_list.add(asteroid) #los que nos chocan volverán a salir
        asteroid.rect.x = random.randrange(1000,2000)
        asteroid.rect.y = random.randrange(40, 570)"""
        if navy.hp <= 0:
           for i in range (9):
            crash = "images/regularExplosion0{}.png".format(i)
            img = pg.image.load(crash).convert()
